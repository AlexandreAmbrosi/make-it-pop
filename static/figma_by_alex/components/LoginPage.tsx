...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigateToSignup, onNavigateToHome }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8 relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full max-w-[446px]">
            ...
            >
              ...
              <div className="text-center space-y-2">
                <h1 className="text-[18px] font-medium text-neutral-950 leading-[16px]">
                  Welcome back, fellow designer!
                </h1>
                <p className="text-[16px] text-[#717182] leading-[24px]">
                  New content is waiting for you :)
                </p>
              </div>
            </motion.div>
                ...
                <div className="relative">
                  ...
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-[#717182] hover:text-neutral-950 transition-colors cursor-pointer"
                      disabled={loading}
                    >
                      {showPassword ? (
            ...
            >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#030213] text-white rounded-[8px] py-2 px-4 flex items-center justify-center gap-2 hover:bg-[#030213]/90 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                {loading ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
                    <span className="text-[14px] font-medium leading-[20px]">Loading...</span>
                  </>
                ) : (
                  <>
                    <span className="text-[14px] font-medium leading-[20px]">Jump in !</span>
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[14px] text-[#717182] leading-[20px]">No account ?</span>
                <button
                  type="button"
                  onClick={onNavigateToSignup}
                  disabled={loading}
                  className="text-[14px] font-medium text-neutral-950 leading-[24px] hover:underline transition-all disabled:opacity-50 cursor-pointer"
                >
                  Sign up for free
                </button>
              </div>
      ...
      <button
        ...
        className="fixed top-6 left-6 flex items-center gap-2 opacity-50 hover:opacity-100 transition-opacity duration-200 z-10 cursor-pointer"
        disabled={loading}
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 16 16">
